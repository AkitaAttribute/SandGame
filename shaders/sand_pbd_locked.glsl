...
layout(set = 0, binding = 1, std430) restrict buffer PreviousPositions { vec4 previous_positions[]; };
layout(set = 0, binding = 2, std430) restrict buffer Velocities { vec4 velocities[]; };
layout(set = 0, binding = 3, std430) restrict buffer Corrections { vec4 corrections[]; };
layout(set = 0, binding = 4, std430) restrict buffer CellCounts { uint cell_counts[]; };
layout(set = 0, binding = 5, std430) restrict buffer CellParticles { uint cell_particles[]; };
layout(set = 0, binding = 6, std430) readonly restrict buffer Commands { vec4 command_data[]; };
layout(set = 0, binding = 7, rgba32f) uniform writeonly image2D position_image;
layout(set = 0, binding = 8, std430) restrict buffer SurfaceBits { uint surface_bits[]; };
layout(set = 0, binding = 9, std430) restrict buffer GrainStates { uint grain_states[]; };
layout(set = 0, binding = 10, std430) restrict buffer GrainSleep { uint grain_sleep[]; };
layout(set = 0, binding = 11, std430) restrict buffer ActiveCount { uint active_count[]; };
layout(set = 0, binding = 12, std430) restrict buffer WakeImpulses { ivec4 wake_impulses[]; };
...
const float SUPPORT_TOLERANCE = 0.012;
...
ivec2 particle_texel(uint id) {
    return ivec2(int(id % pc.texture_width), int(id / pc.texture_width));
}
...
void add_wake_impulse(uint id, vec3 velocity_delta) {
    ivec3 fixed_impulse = ivec3(round(velocity_delta * WAKE_IMPULSE_SCALE));
...
bool apply_external_commands(uint id, inout vec3 p, inout vec3 v) {
    ...
    for (uint command_index = 0u; command_index < pc.command_count; ++command_index) {
        ...
        if (mode < 1.5) {
            ...
            vec3 normal = distance > 1e-5 ? offset / distance : vec3(0.0, 1.0, 0.0);
        ...
        if (mode < 2.5) {
            ...
            if (closing_speed > 0.0) v += normal * closing_speed * 0.12;
            v += normal * (penetration / max(pc.dt, 1e-4)) * 0.028;
            continue;
        }
        ...
        if (lateral_length > 1e-5) {
            ...
            lateral_normal = travel_length > 1e-5 ? -travel / travel_length : vec3(1.0, 0.0, 0.0);
        }
...
void phase_clear_grid(uint id) {
    if (id < grid_cell_count()) cell_counts[id] = 0u;
}
...
void phase_solve(uint id) {
    ...
    float support_distance = diameter + SUPPORT_TOLERANCE;
    float support_distance_squared = support_distance * support_distance;

    for (int oz = -1; oz <= 1; ++oz) {
        int cz = int(own_cell.y) + oz;
        if (cz < 0 || cz >= int(pc.grid_z)) continue;
        for (int ox = -1; ox <= 1; ++ox) {
            int cx = int(own_cell.x) + ox;
            if (cx < 0 || cx >= int(pc.grid_x)) continue;
            ...
            for (uint slot = 0u; slot < count; ++slot) {
                ...
                if (other_id == id || other_id >= pc.particle_count) continue;

                vec3 q = positions[other_id].xyz;
                vec3 delta = q - p;
                float distance_squared = dot(delta, delta);
                if (distance_squared >= support_distance_squared) continue;

                float distance = sqrt(max(distance_squared, 1e-10));
                vec3 normal = distance > 1e-5 ? delta / distance : vec3(1.0, 0.0, 0.0);
                if (normal.y < -0.35) has_support = true;
                ...
                vec3 other_velocity = other_locked ? vec3(0.0) : velocities[other_id].xyz;
                float impact_speed = max(0.0, dot(own_velocity - other_velocity, normal));
                ...
                if (
                    ...
                    vec3 transferred_velocity = normal * min(impact_speed * WAKE_TRANSFER, 4.5);
                    add_wake_impulse(other_id, transferred_velocity);

                    // One active grain may propagate into only one new grain.
                    // This turns the wake budget into a true chain limit instead
                    // of an exponentially branching contact tree.
                    grain_states[id] = 1u;
                    state = 1u;
                }

                vec3 q_previous = previous_positions[other_id].xyz;
                vec3 relative_displacement = (p - p_previous) - (q - q_previous);
                vec3 tangent = relative_displacement - normal * dot(relative_displacement, normal);
                float tangent_length = length(tangent);
                if (tangent_length > 1e-5) {
                    float static_limit = pc.friction * 1.55 * penetration;
                    if (tangent_length <= static_limit) {
                        correction -= tangent * (other_locked ? 0.90 : 0.5);
                    } else {
                        float dynamic_limit = pc.friction * normal_correction;
                        float amount = min(
                            tangent_length * (other_locked ? 0.90 : 0.5),
                            dynamic_limit
                        );
    ...
    if (p.y <= pc.grain_radius + SUPPORT_TOLERANCE) has_support = true;
    ...
    corrections[id] = vec4(
        correction,
        min(contact_count, MAX_DAMPING_CONTACTS) + (has_support ? SUPPORT_MARKER : 0.0)
    );
}
...
void phase_finalize(uint id) {
    ...
    float contacts = has_support ? contact_data - SUPPORT_MARKER : contact_data;
    vec3 velocity = raw_velocity;

    if (contacts > 0.0) {
        vec3 correction_velocity = raw_velocity - old_velocity;
        velocity = old_velocity + correction_velocity * CONTACT_CORRECTION_VELOCITY_TRANSFER;
        float contact_keep = pow(CONTACT_KEEP_PER_CONTACT, min(contacts, MAX_DAMPING_CONTACTS));
    ...
    float rest_threshold = lifetime_expired ? EXPIRED_REST_DISPLACEMENT : REST_DISPLACEMENT;
    ...
    bool hard_supported_expiry = has_support && age >= HARD_SETTLE_STEPS;
...
void phase_clear_surface(uint id) {
    if (id < pc.surface_count) surface_bits[id] = 0u;
}
...
void phase_build_surface(uint id) {
    ...
    if (x < 0 || x >= int(pc.axis_particles) || z < 0 || z >= int(pc.axis_particles)) return;
...
void main() {
    ...
    if (pc.phase == 1u) { phase_clear_grid(id); return; }
    if (pc.phase == 7u) { phase_clear_surface(id); return; }
    if (pc.phase == 11u) { phase_clear_active_count(id); return; }
    if (id >= pc.particle_count) return;
    if (pc.phase == 0u) phase_integrate(id);
    else if (pc.phase == 2u) phase_build_grid(id);
    else if (pc.phase == 3u) phase_solve(id);
    else if (pc.phase == 4u) phase_apply(id);
    else if (pc.phase == 5u) phase_finalize(id);
    else if (pc.phase == 6u) phase_initialize(id);
    else if (pc.phase == 8u) phase_build_surface(id);
}
