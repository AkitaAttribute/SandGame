...
void apply_external_commands(uint id, inout vec3 p, inout vec3 v) {
    for (uint command_index = 0u; command_index < pc.command_count; ++command_index) {
        ...
        if (mode < 2.5) {
            // A rigid body is allowed to disturb the packed bed, but most of
            // the contact reaction is returned to the body on the CPU side.
        ...
        // Walking contact is intentionally low energy. Move the intersected
        // grains a few millimeters without converting that positional change
        // into a large velocity on the next step.
        ...
        float lateral_shift = min(penetration * 0.075, 0.0030);
        float downward_compaction = min(penetration * 0.018, 0.0010);
